...
it('can add the star name and star symbol properly', async() => {
    ...
    let user1 = accounts[1];
    let starId = 10;
    await instance.createStar('DDDStar', starId, {from: user1});

    let TokenName = await instance.name();
    let TokenSymbol = await instance.symbol();
    let DDDStar = await instance.lookUptokenIdToStarInfo(starId);

    assert.equal(TokenName,'GetAStar');
    assert.equal(TokenSymbol,'GSR');
    assert.equal(DDDStar,'DDDStar');
});
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar('TheSun',starId, {from: user1});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user1 = accounts[0];
    let starId = 14;
    await instance.createStar('Polaris',starId, {from: user1});
    let Polaris = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal(Polaris,'Polaris');
});